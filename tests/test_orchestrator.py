...
def test_handle_player_input_recognized_command(mock_st, orchestrator, mock_db_session):
    ...
    mock_map_point = Mock(spec=MapPoint, id=1, name="Forest", description="A dark forest.")
...
def test_handle_player_input_unknown_command(mock_st, orchestrator, mock_db_session):
    ...
    assert any(isinstance(call.args[0], LogEntry) and "Unknown command" in call.args[0].content for call in mock_db_session.add.call_args_list)
    mock_db_session.commit.assert_called() # For warden log
...
def test_handle_player_input_natural_language(mock_st, orchestrator, mock_db_session):
    ...
    mock_db_session.add.assert_called() # For player log
    mock_db_session.commit.assert_called() # For player log
    orchestrator.llm_service.generate_response.assert_called_once_with("I want to open the door.")
    mock_db_session.add.assert_called() # For warden log
    mock_db_session.commit.assert_called() # For warden log

# --- Test _handle_look ---
@patch('core.orchestrator.st')
def test_handle_look_no_args_no_entities_or_items(mock_st, orchestrator, mock_db_session):
    mock_st.session_state = {'character_id': 1}
    mock_character = Mock(spec=GameEntity, id=1)
    ...
    mock_db_session.query.return_value.filter.return_value.first.return_value = mock_character
    mock_db_session.query.return_value.filter.return_value.all.side_effect = [[], []] # No entities, no items
    ...
    assert expected_prompt_part in orchestrator.llm_service.generate_response.call_args[0][0]
...
def test_handle_look_no_args_with_entities(mock_st, orchestrator, mock_db_session):
    ...
    assert "LLM response." in response
    orchestrator.llm_service.generate_response.assert_called_once()
    assert "You see: Goblin." in orchestrator.llm_service.generate_response.call_args[0][0]
...
def test_handle_look_no_args_with_items(mock_st, orchestrator, mock_db_session):
    ...
    assert "LLM response." in response
    orchestrator.llm_service.generate_response.assert_called_once()
    assert "On the ground, you see: Sword." in orchestrator.llm_service.generate_response.call_args[0][0]
...
def test_handle_look_target_entity_found(mock_st, orchestrator, mock_db_session):
    ...
    assert "LLM response." in response
    orchestrator.llm_service.generate_response.assert_called_once()
    assert "Describe Goblin, a Monster." in orchestrator.llm_service.generate_response.call_args[0][0]
...
def test_handle_look_target_item_found(mock_st, orchestrator, mock_db_session):
    ...
    assert "LLM response." in response
    orchestrator.llm_service.generate_response.assert_called_once()
    assert "Describe the item: Rusty Sword." in orchestrator.llm_service.generate_response.call_args[0][0]
