...
def orchestrator(mock_llm_service):
    ...
    with patch("core.orchestrator.world_tools") as mock_world_tools:
        # Mock a sample tool
        mock_world_tools.sample_tool = lambda db, arg: {
            "status": f"tool executed with {arg}"
        }
        # Create the orchestrator instance
        orch = WardenOrchestrator(mock_llm_service)
        # Now, the orchestrator's available_tools will be populated from the mock
        return orch


def test_handle_input_no_tool_chosen(orchestrator, mock_llm_service, db_session):
...
def test_handle_input_tool_succeeds(orchestrator, mock_llm_service, db_session):
    ...
    mock_llm_service.choose_tool.assert_called_once()
    mock_llm_service.synthesize_narrative.assert_called_once()
    # Check that the narrative was synthesized with the correct tool result
    call_args, _ = mock_llm_service.synthesize_narrative.call_args
...
def test_handle_input_tool_fails(orchestrator, mock_llm_service, db_session):
    ...
    with patch.dict(
        ...
        args, kwargs = mock_llm_service.synthesize_narrative.call_args
        ...
        # The initial player log is committed, but the warden log for the failed action should also be there
        assert db_session.query(LogEntry).count() == 2


def test_handle_input_unknown_tool(orchestrator, mock_llm_service, db_session):
    ...
    mock_llm_service.choose_tool.assert_called_once()
    mock_llm_service.synthesize_narrative.assert_not_called()
